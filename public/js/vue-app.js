...
const app = createApp({
    setup() {
        ...
        // 搜索模式与引擎
        const searchMode = ref('site'); // 'site' 或 'external'
        const searchEngines = ref([]); // 可编辑的外部搜索引擎
        ...
        // 监听器
        watch(siteSearchKeyword, (newKeyword) => {
        ...
        const ensureDefaultSearchEngines = async () => {
            try {
        ...
        const selectExternalEngine = (engineId) => {
            selectedEngineId.value = engineId || '';
        };

        const buildExternalSearchUrl = (engine, query) => {
            try {
        ...
        // ================= 搜索引擎管理（CRUD） =================
        const engineModal = reactive({ show: false });
        const engineForm = reactive({ name: '', template: '', icon: '🔍', order: 0 });
        ...
        const moveEngineUp = async (engineId) => {
        ...
        const moveEngineDown = async (engineId) => {
            const list = [...(searchEngines.value || [])];
            const idx = list.findIndex(e => e.id === engineId);
